    ...
    ** requests and responses via FIFO */
    switch( RequestByte )
    {
      ...
      **   other error codes for robustness */
      ...
      case 0x64:
        Calibrate(p_control, p_calibration, p_sensor_state);
        LOG_INFO( "\t> gyro min:" );
        LOG_INFO( "\t> %d, %d, %d",p_calibration->gyro_min[0], p_calibration->gyro_min[1] , p_calibration->gyro_min[2]  );
        LOG_INFO( "\t> gyro max:" );
        LOG_INFO( "\t> %d, %d, %d",p_calibration->gyro_max[0], p_calibration->gyro_max[1] , p_calibration->gyro_max[2]  );
        LOG_INFO( "\t> accel min:" );
        LOG_INFO( "\t> %d, %d, %d",p_calibration->accel_min[0], p_calibration->accel_min[1] , p_calibration->accel_min[2]  );
        LOG_INFO( "\t> accel max:" );
        LOG_INFO( "\t> %d, %d, %d",p_calibration->accel_max[0], p_calibration->accel_max[1] , p_calibration->accel_max[2]  );
        break;
